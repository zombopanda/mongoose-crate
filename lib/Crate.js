  ...
  plugin (schema, options) {
    ...
    schema.methods.attach = function (field, attachment, callback) {
      ...
      if (options.fields[field].processor.willOverwrite(model[field])) {
        tasks.push(next => {
          options.fields[field].processor.remove(options.storage, model[field], doc, next)
        })
      }

      // process the attachment
      tasks.push(next => {
        options.fields[field].processor.process(attachment, options.storage, model[field], doc, next)
      })
      ...
      Object.keys(options.fields).forEach((field) => {
        if (options.fields[field].array) {
          model[field].forEach((arrayField) => {
            tasks.push((callback) => {
              options.fields[field].processor.remove(options.storage, arrayField, doc, callback)
            })
          })
        } else {
          tasks.push((callback) => {
        ...
        if (options.fields[field].array) {
          ...
          model.__cached_attachments[field].forEach((oldDoc) => {
            ...
            if (!present) {
              ...
              tasks.push((callback) => {
                options.fields[field].processor.remove(options.storage, oldDoc, doc, callback)
              })
            }
          })
          ...
          tasks.push((callback) => {
            options.fields[field].processor.remove(options.storage, model.__cached_attachments[field], doc, callback)
          })
        }
      })
